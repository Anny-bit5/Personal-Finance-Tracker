...
def prediction():
    ...
    # Get recent months data (last 6 months) - as list of tuples
    recent_data = []
    recent_months_list = []
    recent_amounts_list = []
    for month_key in sorted(monthly_expenses.keys())[-6:]:
        recent_data.append({
            'month': month_key,
    ...
    # Prepare chart data (historical + predictions)
    chart_labels = [item['month'] for item in recent_data]
    chart_historical = [item['amount'] for item in recent_data]
    chart_predictions = []
    
    if predictions_data:
        for pred in predictions_data:
            # Add prediction month to labels
            chart_labels.append(pred['month'])
            chart_predictions.append(pred['prediction'])
